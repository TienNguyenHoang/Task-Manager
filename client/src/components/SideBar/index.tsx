...
import { UserIcon, DashboardIcon, PendingIcon, CompletedIcon, StarIcon, LightIcon } from '../Icons';
import config from '~/config';

const menuItems = [
    { name: 'Dashboard', icon: DashboardIcon, path: config.routes.dashboard },
    { name: 'Chờ xử lý', icon: PendingIcon, path: config.routes.pending },
    { name: 'Hoàn tất', icon: CompletedIcon, path: config.routes.completed },
];
        ...
        <div className="w-1/6 p-6 shadow-md">
            ...
            <div className="mt-5 rounded-2xl border-2 border-gray-300/10 p-3">
                <div className="text-main flex items-center justify-between">
                    ...
                    <p className="bg-main/10 rounded-2xl p-2 text-xs">50%</p>
                </div>
            ...
            <div className="from-side/5 to-main/5 mt-5 flex items-center gap-2 rounded-2xl bg-gradient-to-r p-5 shadow-md">
                <LightIcon className="text-main size-12 rounded-2xl bg-gray-300/30 p-2" />
